...
def generateWeather(nYears):
    ...
    dates = pandas.date_range(start=datetime(1979,1,1),end=datetime(2018,12,31))
    ...
    for j in range(np.shape(database)[0]):
        data = np.loadtxt('Weather/CRU/txt/lat' + str(np.round(database[j,1],3)) + '_long' + str(np.round(database[j,2],3)) + '.txt', \
            skiprows=1, usecols=[3,4])
                
        if any(np.isnan(data[:,0]))==False and any(np.isnan(data[:,1]))==False:
            # find historical monthly mean temperatures and total precipitation amounts in 3 consecutive 30-day periods post planting
            # first find how many of the 5 30-day periods post plant date are complete and size monthly data matrices accordingly
            # if plant date is Feb 29th, change to Feb 28th since there is no Feb 29th in 2018
            if database[j,3] == 2 and database[j,4] == 29:
                database[j,4] = 28
    
            lastPlantDate = np.where(dates==datetime(2018,int(database[j,3]),int(database[j,4])))[0][0] # planting date in last year
            remainingDays = len(dates) - lastPlantDate + 1 # number of days left in the calendar year
            # initialize all months with 39 years; add an extra year if there are enough remaining days in 2018 for a full month
            month1 = np.zeros([39,2])
            month2 = np.zeros([39,2])
            month3 = np.zeros([39,2])
            month4 = np.zeros([39,2])
            month5 = np.zeros([39,2])
            if remainingDays >= 30: # 1st month has an extra year of data
                month1 = np.zeros([40,2])
            elif remainingDays >= 60: # 1st 2 months have an extra year of data
                month2 = np.zeros([40,2])
            elif remainingDays >= 90: # 1st 3 months have an extra year of data
                month3 = np.zeros([40,2])
            elif remainingDays >= 120: # 1st 4 months have an extra year of data
                month4 = np.zeros([40,2])
            elif remainingDays >= 150: #all months have an extra year of data
                month5 = np.zeros([40,2])
                
            # populate monthly data matrices
            months = [month1, month2, month3, month4, month5]
            nMonths = len(months)
            for k in range(nMonths): # loop through 5 months
                for i in range(np.shape(months[k])[0]): # loop through 39 or 40 years
                    day1 = np.where(dates==datetime(1979+i,int(database[j,3]),int(database[j,4])))[0][0] # planting date in year i
            ...
            for i in range(nMonths):
                ...
                if any(np.isnan(pptPcts)): # occurs because precip is constant every year, so can't fit gamma distn
                    pptPcts[np.where(np.isnan(pptPcts))] = 0.5 # median (which is constant for all percentiles)
            ...
            for i in range(nMonths):
                ...
                if np.isnan(rhos[i]): # no correlation because one variable is constant
                    rhos[i] = 0.0
            ...
            for i in range(len(zs)):
                ...
                if any(np.isnan(precip)):
                    precip[np.where(np.isnan(precip))] = np.median(months[i][:,1]) # replace nan with constant precipitation value
            ...
            for i in range(len(tempData)):
                tempData[i] = tempData[i] + pars[i][0]*(2019-1979) + pars[i][1]
            ...
            np.savetxt('Weather/CRU/synthetic/lat' + str(np.round(database[j,1],3)) + '_long' + str(np.round(database[j,2],3)) + '_temp.txt',
                       tempMatrix)
            ...
            np.savetxt('Weather/CRU/synthetic/lat' + str(np.round(database[j,1],3)) + '_long' + str(np.round(database[j,2],3)) + '_precip.txt',
                       precipMatrix)
...
def compareWeather(months, tempData, precipData, sigmas, alphas, betas, rowNo):
    ...
    fig.savefig('Weather/CRU/validation/Row' + str(rowNo) + '.png')
    fig.clf()
